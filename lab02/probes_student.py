...
def probe_torch(env: Env) -> dict[str, Any]:
    ...
    versions = _split_local_version(str(raw)) if raw else None
    ...
    out = {
        "value": raw,
        "source": src,
        "status": "ok" if versions else "unknown",
        "version": versions,
    ...
    nv = versions["nvidia_build"] if versions else False
...
def probe_tensorrt(env: Env) -> dict[str, Any]:
    ...
    try:
        ...
        hint =""
        prefix = getattr_path(env, "python.prefix")
        base_prefix = getattr_path(env, "python.base_prefix")
        if prefix and base_prefix and prefix != base_prefix:
            hint = " — you are inside a virtual environment, and TensorRT is a system" \
